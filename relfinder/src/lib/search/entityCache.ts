...
/**
 * Creates an isolated entity cache instance. Each call returns a fresh instance
 * with its own in-memory store and IDB connection. Production code uses the
 * module-level singleton exported below; tests call this directly so they get
 * a clean slate each time without needing a `_resetForTest` escape hatch.
 */
export function createEntityCache() {
  ...
  // External modules may wire in here without touching cache internals.
  // Properties on this object are mutable even though the binding is not.
  const hooks: {
    onAdd: ((entities: CachedEntity[]) => void) | null
    onEvict: ((iris: string[]) => void) | null
  } = { onAdd: null, onEvict: null }
   ...
   * Degrades gracefully to in-memory-only when IDB is unavailable.
   */
  function initEntityCache(): Promise<void> {
  ...
  function cacheAdd(
    ...
    for (const entity of entities) {
      ...
      const record: CachedEntity = {
        ...entity,
        // Preserve timestamps from the first insertion.
  ...
  function cacheGet(iri: string): CachedEntity | undefined {
  ...
  function cacheRemove(iris: string[]): void {
    let removed = false
  ...
  function cacheAll(): CachedEntity[] {
  ...
  function cacheHas(iri: string): boolean {
...
// at any call site. Tests use createEntityCache() to get a fresh instance.

const _singleton = createEntityCache()

export const hooks = _singleton.hooks
export const initEntityCache = _singleton.initEntityCache
export const cacheAdd = _singleton.cacheAdd
export const cacheGet = _singleton.cacheGet
export const cacheRemove = _singleton.cacheRemove
export const cacheAll = _singleton.cacheAll
export const cacheSize = _singleton.cacheSize
export const cacheHas = _singleton.cacheHas
export const cacheClear = _singleton.cacheClear
